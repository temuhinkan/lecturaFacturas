...
"""
    ...
    def _create_viewer_panel(self, parent):
        ...
        # 🚨 CAMBIO 1: CREAR EL MARCO PRINCIPAL DEL VISOR (self.viewer_frame)
        # Esto debe ir PRIMERO para que los controles puedan usarlo como padre.
        self.viewer_frame = ttk.Frame(parent)
        self.viewer_frame.pack(side=tk.TOP, fill=tk.BOTH, expand=True)

        # 🚨 CAMBIO 2: CREAR EL MARCO DE CONTROLES DEL VISOR (self.viewer_controls_frame)
        # Este marco se usa para los botones de rotación y paginación.
        self.viewer_controls_frame = ttk.Frame(self.viewer_frame)
        self.viewer_controls_frame.pack(side=tk.TOP, fill=tk.X, pady=5)
        
        # Frame contenedor para el canvas y scrollbars
        # Usamos self.viewer_frame como padre, en lugar de 'parent' directamente
        canvas_container = ttk.Frame(self.viewer_frame) 
        canvas_container.pack(fill=tk.BOTH, expand=True, pady=5)
        
        # 1. Panel de Control de Selección de Texto (Se mantiene como hijo de 'parent')
        ...
        # 🚨 ATENCIÓN: Esta línea ya no es necesaria y se quita para evitar duplicar el empaquetado del contenedor
        # canvas_container.pack(fill=tk.BOTH, expand=True, pady=5) 
        ...
        # 🟢 NUEVO: Botón de Validar Factura
        validate_control_frame = ttk.Frame(control_frame)
        validate_control_frame.pack(fill=tk.X, pady=5)
        
        self.validate_button = ttk.Button(validate_control_frame, 
                                         text="✅ Validar Factura (Marcar como Correcta en BBDD)", 
                                         command=self._validate_invoice,
                                         style='Accent.TButton' 
                                         )
        self.validate_button.pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
        ...
        ttk.Button(page_zoom_frame, text="➡️", command=lambda: self.change_page(-1)).pack(side=tk.RIGHT, padx=(5, 5))
        ...
        ttk.Button(page_zoom_frame, text="⬅️", command=lambda: self.change_page(1)).pack(side=tk.RIGHT, padx=(5, 0))
        
        # 🚨 CAMBIO 3: INSERCIÓN DE BOTONES DE ROTACIÓN Y ELIMINACIÓN DE DUPLICADOS
        # Estos botones se añaden al principio del self.viewer_controls_frame
        ttk.Button(self.viewer_controls_frame, text="⟲ Girar Izq. (90°)", 
                   command=lambda: self._rotate_document(-90)).pack(side=tk.LEFT, padx=5, pady=5)
        ttk.Button(self.viewer_controls_frame, text="⟳ Girar Der. (90°)", 
                   command=lambda: self._rotate_document(90)).pack(side=tk.LEFT, padx=5, pady=5)
                   
        # 🚨 CAMBIO 4: ELIMINAR LÍNEAS DUPLICADAS O FUERA DE LUGAR
        # Las siguientes líneas estaban causando el error y deben eliminarse del final:
        # self.viewer_frame = ttk.Frame(parent_frame)  # Duplicado
        # self.viewer_frame.pack(side=tk.TOP, fill=tk.BOTH, expand=True) # Duplicado
        # self.viewer_controls_frame = ttk.Frame(self.viewer_frame) # Duplicado
        # self.viewer_controls_frame.pack(side=tk.TOP, fill=tk.X, pady=5) # Duplicado
        # viewer_controls_frame = ttk.Frame(control_frame) # Duplicado
        # viewer_controls_frame.pack(fill=tk.X, pady=5) # Duplicado
        
        # 🚨 NOTA FINAL: El código de los botones de rotación se movió al inicio del control_frame
        # para que se ejecute después de crear self.viewer_controls_frame
        # El código de los botones de rotación estaba duplicado y se ha movido al principio
        # del panel de control de la aplicación, como se indica en el CAMBIO 3
    ...
    def _rotate_document(self, angle: int):
        """
        Gira la imagen de la página en el visor y actualiza el ángulo.
        """
        # 1. Aplicar la rotación (módulo 360)
        # Ejemplo: Si el ángulo actual es 270 y giramos +90, el nuevo ángulo es 0.
        self.rotation_angle = (self.rotation_angle + angle) % 360
        
        # 2. Recargar la página con el nuevo ángulo
        self._display_page()
        
        print(f"Rotación actual: {self.rotation_angle}°")
    ...
    def _on_selection_release(self, event):
        ...
        # Restablecer el inicio de la selección si las coordenadas son nulas
        if self.start_x is None or self.start_y is None:
            self.canvas.delete("selection_rect")
            return

        # 1. Borrar el rectángulo de selección temporal 
        self.canvas.delete("selection_rect")
        self.selection_rect_id = None
        ...
        # --- LÓGICA DE EXTRACCIÓN AÑADIDA/CORREGIDA ---
        
        # 3. Convertir a coordenadas PDF (escalado inverso: dividir por el zoom)
        page = self.doc[self.page_num]
        ...
        # 4. Extraer palabras dentro del rectángulo de PDF
        # get_text('words') devuelve (x0, y0, x1, y1, 'word', block_no, line_no, word_no)
        words_in_rect = page.get_text("words", clip=rect_pdf, sort=True)
        
        if not words_in_rect:
            self.selected_word = None
            self.selected_line_content = None
            self.word_var.set("[Ninguna palabra seleccionada]")
            self.line_ref_var.set("")
            return

        # 5. Reconstruir la palabra/frase seleccionada
        self.selected_word = " ".join([w[4] for w in words_in_rect])
        
        # 6. Encontrar la línea completa de la primera palabra seleccionada
        # Para encontrar la línea completa, extraemos el texto del área
        full_page_text = page.get_text("text")
        
        # Buscamos la palabra/frase seleccionada y extraemos la línea completa
        # Esto es una simplificación, ya que el texto en fitz puede no tener saltos de línea consistentes.
        # Pero es un buen punto de partida.
        self.selected_line_content = None
        
        # Intentamos encontrar una línea que contenga la primera palabra seleccionada
        first_word = words_in_rect[0][4]
        for line in full_page_text.splitlines():
            if first_word in line:
                self.selected_line_content = line.strip()
                break

        # 7. Actualizar las variables de la GUI
        self.word_var.set(f"✅ Palabra/Valor: {self.selected_word}")
        
        if self.selected_line_content:
            self.line_ref_var.set(f"🔗 Línea Referencia: {self.selected_line_content}")
        else:
            self.line_ref_var.set("⚠️ No se pudo determinar la línea de referencia.")

        # Opcional: Centrar la regla de edición en la pestaña Editor de Reglas
        if self.active_data_field and self.notebook.tab(self.notebook.select(), "text") == 'Editor de Reglas de Extracción':
            # Intentar actualizar los campos del editor de reglas si están visibles
            self._update_rule_editor_with_selection() 
            
        # --- FIN LÓGICA DE EXTRACCIÓN AÑADIDA/CORREGIDA ---

        # Resetear coordenadas de inicio
        self.start_x = None
        self.start_y = None
    ...
    def _validate_invoice(self):
        """
        Marca la factura actualmente cargada como validada (is_validated = 1) en la BBDD.
        """
        # CRÍTICO: El file_path debe estar almacenado como atributo de la clase
        if not hasattr(self, 'file_path') or not self.file_path:
            messagebox.showwarning("Advertencia", "No hay una factura cargada para validar.")
            return

        invoice_path = self.file_path
        
        # Confirmación
        if not messagebox.askyesno("Confirmar Validación", 
                                   f"¿Está seguro que desea marcar la factura '{os.path.basename(invoice_path)}' como VALIDADA?"):
            return

        try:
            # 1. Llamada a la función de la base de datos
            # Usamos update_invoice_field para cambiar 'is_validated' a 1.
            rows_affected = database.update_invoice_field(
            ...
            if rows_affected > 0:
                messagebox.showinfo("Validación Exitosa", 
                                    f"Factura '{os.path.basename(invoice_path)}' marcada como validada (is_validated=1).")
                
                # Deshabilitar el botón de validar tras la acción exitosa
            ...
            messagebox.showerror("Error", f"Ocurrió un error al validar la factura: {e}")
